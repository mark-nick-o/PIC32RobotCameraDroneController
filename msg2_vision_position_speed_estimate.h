#ifndef __mav2_vis_posn_speed_est__
#define __mav2_vis_posn_speed_est__
...
#define MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE 103u

#if defined(D_FT900)
typedef struct __mavlink_vision_speed_estimate_t MAVPACKED {
 uint64_t usec;                                                                 /*< [us] Timestamp (UNIX time or time since system boot)*/
 float32_t x;                                                                   /*< [m/s] Global X speed*/
 float32_t y;                                                                   /*< [m/s] Global Y speed*/
 float32_t z;                                                                   /*< [m/s] Global Z speed*/
 float32_t covariance[9u];                                                      /*<  Row-major representation of 3x3 linear velocity covariance matrix (states: vx, vy, vz; 1st three entries - 1st row, etc.). If unknown, assign NaN value to first element in the array.*/
...
#define MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN 57u
#define MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_MIN_LEN 20u
#define MAVLINK_MSG_ID_103_LEN 57u
#define MAVLINK_MSG_ID_103_MIN_LEN 20u

#define MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_CRC 208u
#define MAVLINK_MSG_ID_103_CRC 208u

#define MAVLINK_MSG_VISION_SPEED_ESTIMATE_FIELD_COVARIANCE_LEN 9u
...
static inline uint16_t mavlink_msg_vision_speed_estimate_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint64_t usec, float32_t x, float32_t y, float32_t z, const float32_t *covariance, uint8_t reset_counter)
{
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_vision_speed_estimate_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, uint64_t usec,float32_t x,float32_t y,float32_t z,const float32_t *covariance,uint8_t reset_counter)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN];
    _mav_put_uint64_t(buf, 0, usec);
...
static inline void mavlink_msg_vision_speed_estimate_send(mavlink_channel_t chan, uint64_t usec, float32_t x, float32_t y, float32_t z, const float32_t *covariance, uint8_t reset_counter)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN];
    _mav_put_uint64_t(buf, 0, usec);
 ...
 */
static inline void mavlink_msg_vision_speed_estimate_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t usec, float32_t x, float32_t y, float32_t z, const float32_t *covariance, uint8_t reset_counter)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 */
static inline float32_t mavlink_msg_vision_speed_estimate_get_x(const mavlink_message_t* msg)
{
 ...
 */
static inline float32_t mavlink_msg_vision_speed_estimate_get_y(const mavlink_message_t* msg)
{
 ...
 */
static inline float32_t mavlink_msg_vision_speed_estimate_get_z(const mavlink_message_t* msg)
{
 ...
 */
static inline uint16_t mavlink_msg_vision_speed_estimate_get_covariance(const mavlink_message_t* msg, float32_t *covariance)
{
    return _MAV_RETURN_float32_t_array(msg, covariance, 9,  20);
}
...
static inline void mavlink_msg_vision_speed_estimate_decode(const mavlink_message_t* msg, mavlink_vision_speed_estimate_t* vision_speed_estimate)
{
    ...
    memcpy((void*)vision_speed_estimate,(void*)_MAV_PAYLOAD(msg), len);
#endif
}
