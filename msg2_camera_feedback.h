#ifndef __cam_fb_mavlink2__
#define __cam_fb_mavlink2__
...
#define MAVLINK_MSG_ID_CAMERA_FEEDBACK 180u

#if defined(D_FT900)
typedef struct __mavlink_camera_feedback_t MAVPACKED {
 ...
 float32_t alt_msl;                                                             /*< [m] Altitude (MSL).*/
 float32_t alt_rel;                                                             /*< [m] Altitude (Relative to HOME location).*/
 float32_t roll;                                                                /*< [deg] Camera Roll angle (earth frame, +-180).*/
 float32_t pitch;                                                               /*< [deg] Camera Pitch angle (earth frame, +-180).*/
 float32_t yaw;                                                                 /*< [deg] Camera Yaw (earth frame, 0-360, true).*/
 float32_t foc_len;                                                             /*< [mm] Focal Length.*/
...
#define MAVLINK_MSG_ID_CAMERA_FEEDBACK_LEN 47u
#define MAVLINK_MSG_ID_CAMERA_FEEDBACK_MIN_LEN 45u
#define MAVLINK_MSG_ID_180_LEN 47u
#define MAVLINK_MSG_ID_180_MIN_LEN 45u

#define MAVLINK_MSG_ID_CAMERA_FEEDBACK_CRC 52u
#define MAVLINK_MSG_ID_180_CRC 52u
...
static inline uint16_t mavlink_msg_camera_feedback_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint64_t time_usec, uint8_t target_system, uint8_t cam_idx, uint16_t img_idx, int32_t lat, int32_t lng, float32_t alt_msl, float32_t alt_rel, float32_t roll, float32_t pitch, float32_t yaw, float32_t foc_len, uint8_t flags, uint16_t completed_captures)
{
...
static inline void mavlink_msg_camera_feedback_send(mavlink_channel_t chan, uint64_t time_usec, uint8_t target_system, uint8_t cam_idx, uint16_t img_idx, int32_t lat, int32_t lng, float32_t alt_msl, float32_t alt_rel, float32_t roll, float32_t pitch, float32_t yaw, float32_t foc_len, uint8_t flags, uint16_t completed_captures)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_CAMERA_FEEDBACK_LEN];
    _mav_put_uint64_t(buf, 0, time_usec);
 ...
 */
static inline void mavlink_msg_camera_feedback_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t time_usec, uint8_t target_system, uint8_t cam_idx, uint16_t img_idx, int32_t lat, int32_t lng, float32_t alt_msl, float32_t alt_rel, float32_t roll, float32_t pitch, float32_t yaw, float32_t foc_len, uint8_t flags, uint16_t completed_captures)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
static inline void mavlink_msg_camera_feedback_decode(const mavlink_message_t* msg, mavlink_camera_feedback_t* camera_feedback)
{
    ...
    memset(camera_feedback, 0u, MAVLINK_MSG_ID_CAMERA_FEEDBACK_LEN);
    memcpy((void*) camera_feedback,(void*) _MAV_PAYLOAD(msg),(int16_t) len);
#endif
}
