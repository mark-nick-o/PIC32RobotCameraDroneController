#ifndef __mav1_vis_speed_est__
#define __mav1_vis_speed_est__
//#pragma once
...
#if defined(D_FT900)
typedef struct mavlink_vision_speed_estimate_t MAVPACKED {
...
#define MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN 20u
#define MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_MIN_LEN 20u
#define MAVLINK_MSG_ID_103_LEN 20u
 ...
 */
static inline uint16_t mavlink_msg_vision_speed_estimate_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint64_t usec, float32_t x, float32_t y, float32_t z)
{
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_vision_speed_estimate_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, uint64_t usec,float32_t x,float32_t y,float32_t z)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN];
    _mav_put_uint64_t(buf, 0, usec);
...
static inline void mavlink_msg_vision_speed_estimate_send(mavlink_channel_t chan, uint64_t usec, float32_t x, float32_t y, float32_t z)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN];
    _mav_put_uint64_t(buf, 0, usec);
 ...
 */
static inline void mavlink_msg_vision_speed_estimate_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t usec, float32_t x, float32_t y, float32_t z)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
