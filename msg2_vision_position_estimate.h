#ifndef __mav2_vis_posn_est__
#define __mav2_vis_posn_est__
...
#define MAVLINK_MSG_ID_VISION_POSITION_ESTIMATE 102u

#if defined(D_FT900)
typedef struct __mavlink_vision_position_estimate_t MAVPACKED {
 uint64_t usec;                                                                 /*< [us] Timestamp (UNIX time or time since system boot)*/
 float32_t x;                                                                   /*< [m] Global X position*/
 float32_t y;                                                                   /*< [m] Global Y position*/
 float32_t z;                                                                   /*< [m] Global Z position*/
 float32_t roll;                                                                /*< [rad] Roll angle*/
 float32_t pitch;                                                               /*< [rad] Pitch angle*/
 float32_t yaw;                                                                 /*< [rad] Yaw angle*/
 float32_t covariance[21u];                                                     /*<  Row-major representation of pose 6x6 cross-covariance matrix upper right triangle (states: x, y, z, roll, pitch, yaw; first six entries are the first ROW, next five entries are the second ROW, etc.). If unknown, assign NaN value to first element in the array.*/
...
#define MAVLINK_MSG_ID_VISION_POSITION_ESTIMATE_LEN 117u
#define MAVLINK_MSG_ID_VISION_POSITION_ESTIMATE_MIN_LEN 32u
#define MAVLINK_MSG_ID_102_LEN 117u
#define MAVLINK_MSG_ID_102_MIN_LEN 32u

#define MAVLINK_MSG_ID_VISION_POSITION_ESTIMATE_CRC 158u
#define MAVLINK_MSG_ID_102_CRC 158u

#define MAVLINK_MSG_VISION_POSITION_ESTIMATE_FIELD_COVARIANCE_LEN 21u
 ...
 */
static inline uint16_t mavlink_msg_vision_position_estimate_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint64_t usec, float32_t x, float32_t y, float32_t z, float32_t roll, float32_t pitch, float32_t yaw, const float32_t *covariance, uint8_t reset_counter)
{
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_vision_position_estimate_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, uint64_t usec,float32_t x,float32_t y,float32_t z,float32_t roll,float32_t pitch,float32_t yaw,const float32_t *covariance,uint8_t reset_counter)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_VISION_POSITION_ESTIMATE_LEN];
    _mav_put_uint64_t(buf, 0, usec);
    ...
    packet.yaw = yaw;
    packet.reset_counter = reset_counter;
    mav_array_memcpy(packet.covariance, covariance, sizeof(float32_t)*21);
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), &packet, MAVLINK_MSG_ID_VISION_POSITION_ESTIMATE_LEN);
#endif
...
static inline void mavlink_msg_vision_position_estimate_send(mavlink_channel_t chan, uint64_t usec, float32_t x, float32_t y, float32_t z, float32_t roll, float32_t pitch, float32_t yaw, const float32_t *covariance, uint8_t reset_counter)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_VISION_POSITION_ESTIMATE_LEN];
    _mav_put_uint64_t(buf, 0, usec);
 ...
 */
static inline void mavlink_msg_vision_position_estimate_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t usec, float32_t x, float32_t y, float32_t z, float32_t roll, float32_t pitch, float32_t yaw, const float32_t *covariance, uint8_t reset_counter)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 */
static inline float32_t mavlink_msg_vision_position_estimate_get_z(const mavlink_message_t* msg)
{
 ...
 */
static inline float32_t mavlink_msg_vision_position_estimate_get_roll(const mavlink_message_t* msg)
{
 ...
 */
static inline float32_t mavlink_msg_vision_position_estimate_get_pitch(const mavlink_message_t* msg)
{
 ...
 */
static inline float32_t mavlink_msg_vision_position_estimate_get_yaw(const mavlink_message_t* msg)
{
 ...
 */
static inline uint16_t mavlink_msg_vision_position_estimate_get_covariance(const mavlink_message_t* msg, float32_t *covariance)
{
    return _MAV_RETURN_float32_t_array(msg, covariance, 21,  32);
}
...
static inline void mavlink_msg_vision_position_estimate_decode(const mavlink_message_t* msg, mavlink_vision_position_estimate_t* vision_position_estimate)
{
    ...
    memcpy((void*)vision_position_estimate,(void*) _MAV_PAYLOAD(msg), len);
#endif
}
