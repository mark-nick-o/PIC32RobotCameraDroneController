#ifndef __mav2_vicon_data_stream__
#define __mav2_vicon_data_stream__
...
#define MAVLINK_MSG_ID_VICON_POSITION_ESTIMATE 104u
...
typedef struct MAVPACKED {
 ...
 float32_t covariance[21u];                                                     /*<  Row-major representation of 6x6 pose cross-covariance matrix upper right triangle (states: x, y, z, roll, pitch, yaw; first six entries are the first ROW, next five entries are the second ROW, etc.). If unknown, assign NaN value to first element in the array.*/
...
#define MAVLINK_MSG_ID_VICON_POSITION_ESTIMATE_LEN 116u
#define MAVLINK_MSG_ID_VICON_POSITION_ESTIMATE_MIN_LEN 32u
#define MAVLINK_MSG_ID_104_LEN 116u
#define MAVLINK_MSG_ID_104_MIN_LEN 32u

#define MAVLINK_MSG_ID_VICON_POSITION_ESTIMATE_CRC 56u
#define MAVLINK_MSG_ID_104_CRC 56u

#define MAVLINK_MSG_VICON_POSITION_ESTIMATE_FIELD_COVARIANCE_LEN 21u
...
static inline uint16_t mavlink_msg_vicon_position_estimate_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint64_t usec, float32_t x, float32_t y, float32_t z, float32_t roll, float32_t pitch, float32_t yaw, const float32_t *covariance)
{
...
static inline void mavlink_msg_vicon_position_estimate_send(mavlink_channel_t chan, uint64_t usec, float32_t x, float32_t y, float32_t z, float32_t roll, float32_t pitch, float32_t yaw, const float32_t *covariance)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_VICON_POSITION_ESTIMATE_LEN];
    _mav_put_uint64_t(buf, 0, usec);
 ...
 */
static inline void mavlink_msg_vicon_position_estimate_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t usec, float32_t x, float32_t y, float32_t z, float32_t roll, float32_t pitch, float32_t yaw, const float32_t *covariance)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 */
static inline uint16_t mavlink_msg_vicon_position_estimate_get_covariance(const mavlink_message_t* msg, float32_t *covariance)
{
...
static inline void mavlink_msg_vicon_position_estimate_decode(const mavlink_message_t* msg, mavlink_vicon_position_estimate_t* vicon_position_estimate)
{
    ...
    memcpy((void*)vicon_position_estimate,(void*) _MAV_PAYLOAD(msg),(int16_t) len);
#endif
}
