...
/* Hand-Eye Begin */
#define API_GOGN_HEB_CAL(X, camId)  do{ sprintf(X,"HEB,%u",camId); } while(0)
                      
/* ====================== SLMP data send ===================================  */
#define COGN_HEB_SLMP_CMD 2020u
#define COGN_HEB_SND_OFFST 2u*COGN_WORD_LEN                                     /* offset for memcpy */

#if defined(D_FT900)
typedef struct COGPACKED {
    uint16_t cmdId;                                                              /* set to COGN_HEB_SLMP_CMD at initialisation */
    uint32_t camId;
} COGN_HEB_CAL_Snd_t;
#else
COGPACKED(
...
#define COGN_HEB_RCV_OFFST 5u*COGN_WORD_LEN                                      /* offset for memcpy */
...
/* ================= Hand Eye Execute Step ============  */

/* tcp/ip ascii send */

/* Hand-Eye eXecute */
#define API_GOGN_HE_CAL(X, camId, feature, pose)  do{ sprintf(X,"HE,%u,%d,%d,%d,%d,%d,%d,%d",camId,feature,pose.coord.x,pose.coord.y,pose.coord.z,pose.rotat.z,pose.rotat.y,pose.rotat.x); } while(0)
                      
/* ====================== SLMP data send ===================================  */
#define COGN_HE_SLMP_CMD 2021u
#define COGN_HE_SND_OFFST 2u*COGN_WORD_LEN                                     /* offset for memcpy */

#if defined(D_FT900)
typedef struct COGPACKED {
    uint16_t cmdId;                                                              /* set to COGN_HE_SLMP_CMD at initialisation */
    uint32_t camId;
    uint32_t featureId;
    uint32_t coordX;
    uint32_t coordY;
...
#define COGN_HE_RCV_OFFST 5u*COGN_WORD_LEN                                      /* offset for memcpy */
...
/* Hand-Eye eNd */
#define API_GOGN_HEE_CAL(X, camId)  do{ sprintf(X,"HEE,%u",camId); } while(0)
                      
/* ====================== SLMP data send ===================================  */
#define COGN_HEE_SLMP_CMD 2022u
#define COGN_HEE_SND_OFFST 2u*COGN_WORD_LEN                                     /* offset for memcpy */

#if defined(D_FT900)
typedef struct COGPACKED {
    uint16_t cmdId;                                                              /* set to COGN_HEE_SLMP_CMD at initialisation */
    uint32_t camId;        
} COGN_HEE_CAL_Snd_t;
#else
COGPACKED(
...
#define COGN_HEE_RCV_OFFST 5u*COGN_WORD_LEN                                      /* offset for memcpy */
...
/* ================= Custom Calibration End ============  */

/* tcp/ip ascii send */

/* Custom Calibration eNd */
#define API_GOGN_CCE_CAL(X, camId)  do{ sprintf(X,"CCE,%u",camId); } while(0)
                      
/* ====================== SLMP data send ===================================  */
#define COGN_CCE_SLMP_CMD 2042u
#define COGN_CCE_SND_OFFST 2u*COGN_WORD_LEN                                     /* offset for memcpy */

#if defined(D_FT900)
typedef struct COGPACKED {
    uint16_t cmdId;                                                              /* set to COGN_CCE_SLMP_CMD at initialisation */
    uint32_t camId;        
} COGN_CCE_CAL_Snd_t;
#else
COGPACKED(
...
#define COGN_CCE_RCV_OFFST 5u*COGN_WORD_LEN                                      /* offset for memcpy */
...
/* ================= Custom Calibration Begin ============  */

/* tcp/ip ascii send */

/* Custom Calibration Begin */
#define API_GOGN_CCB_CAL(X, camId, swap, x, y)  do{ sprintf(X,"CCB,%u,%d,%d,%d",camId, swap, x, y); } while(0)
                      
/* ====================== SLMP data send ===================================  */
#define COGN_CCB_SLMP_CMD 2040u
#define COGN_CCB_SND_OFFST 2u*COGN_WORD_LEN                                     /* offset for memcpy */

#if defined(D_FT900)
typedef struct COGPACKED {
    uint16_t cmdId;                                                              /* set to COGN_CCB_SLMP_CMD at initialisation */
    uint32_t camId;        
    uint32_t swapHand;
    uint32_t offsetX;
    uint32_t offsetY;
...
#define COGN_CCB_RCV_OFFST 5u*COGN_WORD_LEN                                      /* offset for memcpy */
...
/* ================= Custom Calibration eXecute ============  */

/* tcp/ip ascii send */

/* Custom Calibration eXecute */
#define API_GOGN_CC_CAL(X, camId, swap, x, y)  do{ sprintf(X,"CC,%u,%d,%d,%d",camId, swap, x, y); } while(0)
                      
/* ====================== SLMP data send ===================================  */
#define COGN_CC_SLMP_CMD 2040u
#define COGN_CC_SND_OFFST 2u*COGN_WORD_LEN                                     /* offset for memcpy */

#if defined(D_FT900)
typedef struct COGPACKED {
    uint16_t cmdId;                                                              /* set to COGN_CC_SLMP_CMD at initialisation */
    uint32_t camId;        
    uint32_t swapHand;
    uint32_t offsetX;
    uint32_t offsetY;
...
#define COGN_CC_RCV_OFFST 5u*COGN_WORD_LEN                                      /* offset for memcpy */
...
/* ================= Auto-Calibration Execute Step ============  */

/* tcp/ip ascii send */

/* Auto Calibration eXecute */
#define API_GOGN_AC_CAL(X, camId, feature, pose)  do{ sprintf(X,"AC,%u,%d,%d,%d,%d,%d,%d,%d",camId,feature,pose.coord.x,pose.coord.y,pose.coord.z,pose.rotat.z,pose.rotat.y,pose.rotat.x); } while(0)                      
/* ====================== SLMP data send ===================================  */
#define COGN_AC_SLMP_CMD 2031u
#define COGN_AC_SND_OFFST 2u*COGN_WORD_LEN                                     /* offset for memcpy */

#if defined(D_FT900)
typedef struct COGPACKED {
    uint16_t cmdId;                                                              /* set to COGN_AC_SLMP_CMD at initialisation */
    uint32_t camId;
    uint32_t featureId;
    uint32_t coordX;
    uint32_t coordY;
...
#define COGN_AC_RCV_OFFST 5u*COGN_WORD_LEN                                      /* offset for memcpy */
...
extern uint8_t COGN_HEB_CAL_Receive( char *msg, COGN_HEB_CAL_Rcv_t *rcv );
extern uint8_t COGN_HEB_CAL_Send( char *msg, const uint32_t camId );
extern void COGN_SendHEB( char *msg, const uint32_t camID );
extern uint8_t COGN_ParseHEB_AlignmentReply(char* msg, COGN_HEB_CAL_Rcv_t *poseRead);
extern uint8_t COGN_ParseHE_AlignmentReply(char* msg, COGN_HE_CAL_Rcv_t *poseRead);
extern void COGN_SendHE( char *msg, const uint32_t camID, const uint32_t feature, const COGN_pose_t pose );
extern uint8_t COGN_HE_CAL_Send( char *msg, const uint32_t camId, const uint32_t feature, const COGN_pose_t *pose );
extern uint8_t COGN_HE_CAL_Receive( char *msg, COGN_HE_CAL_Rcv_t *rcv );
extern uint8_t COGN_ParseHEE_AlignmentReply(char* msg, COGN_HEE_CAL_Rcv_t *poseRead);
extern void COGN_SendHEE( char *msg, const uint32_t camID );
extern uint8_t COGN_HEE_CAL_Send( char *msg, const uint32_t camId );
extern uint8_t COGN_HEE_CAL_Receive( char *msg, COGN_HEE_CAL_Rcv_t *rcv );
extern uint8_t COGN_ParseCCB_AlignmentReply(char* msg, COGN_CCB_CAL_Rcv_t *poseRead);
extern void COGN_SendCCB( char *msg, const uint32_t camID );
extern uint8_t COGN_CCB_CAL_Send( char *msg, const uint32_t camId );
extern uint8_t COGN_CCB_CAL_Receive( char *msg, COGN_CCB_CAL_Rcv_t *rcv );
extern uint8_t COGN_ParseCCE_AlignmentReply(char* msg, COGN_CCE_CAL_Rcv_t *poseRead);
extern void COGN_SendCCE( char *msg, const uint32_t camID );
extern uint8_t COGN_CCE_CAL_Send( char *msg, const uint32_t camId );
extern uint8_t COGN_CCE_CAL_Receive( char *msg, COGN_CCE_CAL_Rcv_t *rcv );
extern uint8_t COGN_ParseCC_AlignmentReply(char* msg, COGN_CC_CAL_Rcv_t *poseRead);
extern void COGN_SendCC( char *msg, const uint32_t camID, const uint32_t hand, const uint32_t x, const uint32_t y );
extern uint8_t COGN_CC_CAL_Send( char *msg, const uint32_t camId, const uint32_t hand, const uint32_t x, const uint32_t y );
extern uint8_t COGN_CC_CAL_Receive( char *msg, COGN_CC_CAL_Rcv_t *rcv );
extern uint8_t COGN_ParseAC_AlignmentReply(char* msg, COGN_AC_CAL_Rcv_t *poseRead);
extern void COGN_SendAC( char *msg, const uint32_t camID, const uint32_t feature, const COGN_pose_t pose );
extern uint8_t COGN_AC_CAL_Send( char *msg, const uint32_t camId, const uint32_t feat, const COGN_pose_t *pose );
extern uint8_t COGN_AC_CAL_Receive( char *msg, COGN_AC_CAL_Rcv_t *rcv );
