#ifndef __sensor_dist_mavlink2__
#define __sensor_dist_mavlink2__
...
#define MAVLINK_MSG_ID_DISTANCE_SENSOR 132u

#if defined(D_FT900)
typedef struct __mavlink_distance_sensor_t MAVPACKED {
 ...
 float32_t horizontal_fov;                                                      /*< [rad] Horizontal Field of View (angle) where the distance measurement is valid and the field of view is known. Otherwise this is set to 0.*/
 float32_t vertical_fov;                                                        /*< [rad] Vertical Field of View (angle) where the distance measurement is valid and the field of view is known. Otherwise this is set to 0.*/
 float32_t quaternion[4u];                                                       /*<  Quaternion of the sensor orientation in vehicle body frame (w, x, y, z order, zero-rotation is 1, 0, 0, 0). Zero-rotation is along the vehicle body x-axis. This field is required if the orientation is set to MAV_SENSOR_ROTATION_CUSTOM. Set it to 0 if invalid."*/
...
#define MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN 38u
#define MAVLINK_MSG_ID_DISTANCE_SENSOR_MIN_LEN 14u
#define MAVLINK_MSG_ID_132_LEN 38u
#define MAVLINK_MSG_ID_132_MIN_LEN 14u

#define MAVLINK_MSG_ID_DISTANCE_SENSOR_CRC 85u
#define MAVLINK_MSG_ID_132_CRC 85u

#define MAVLINK_MSG_DISTANCE_SENSOR_FIELD_QUATERNION_LEN 4u
...
static inline uint16_t mavlink_msg_distance_sensor_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint32_t time_boot_ms, uint16_t min_distance, uint16_t max_distance, uint16_t current_distance, uint8_t type, uint8_t id, uint8_t orientation, uint8_t covariance, float32_t horizontal_fov, float32_t vertical_fov, const float32_t *quaternion)
{
...
static inline void mavlink_msg_distance_sensor_send(mavlink_channel_t chan, uint32_t time_boot_ms, uint16_t min_distance, uint16_t max_distance, uint16_t current_distance, uint8_t type, uint8_t id, uint8_t orientation, uint8_t covariance, float32_t horizontal_fov, float32_t vertical_fov, const float32_t *quaternion)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN];
    _mav_put_uint32_t(buf, 0, time_boot_ms);
    ...
    packet.horizontal_fov = horizontal_fov;
    packet.vertical_fov = vertical_fov;
    mav_array_memcpy(packet.quaternion, quaternion, sizeof(float32_t)*4);
...
{
    return _MAV_RETURN_float32_t(msg,  14);
}
...
{
    return _MAV_RETURN_float32_t(msg,  18);
}
...
{
    return _MAV_RETURN_float32_t_array(msg, quaternion, 4,  22);
}
...
static inline void mavlink_msg_distance_sensor_decode(const mavlink_message_t* msg, mavlink_distance_sensor_t* distance_sensor)
{
    ...
    memset(distance_sensor, 0u, MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN);
    memcpy((void*)distance_sensor,(void*) _MAV_PAYLOAD(msg),(int16_t) len);
#endif
}
